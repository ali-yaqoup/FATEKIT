...
export function HomepageAdminForm({ content, featured }: { content: any, featured: any[] }) {
  ...
  return (
    <div className="space-y-8">
      <div className="bg-[#141414] border border-neutral-800 p-6 rounded-xs">
        ...
        <form onSubmit={handleContentSubmit} className="space-y-4">
          <div>
            ...
            {content?.heroImageUrl && (
              <img src={content.heroImageUrl} className="h-32 object-cover rounded-xs border border-neutral-800 mb-2" alt="Hero" />
            )}
          ...
          <div>
            ...
            <input type="text" name="heroTitle" defaultValue={content?.heroTitle} className="w-full bg-neutral-900 border border-neutral-800 p-2 text-sm rounded-xs text-white" />
          </div>
          <div>
            <label className="block text-xs text-neutral-400 mb-1">العنوان الفرعي</label>
            <input type="text" name="heroSubtitle" defaultValue={content?.heroSubtitle} className="w-full bg-neutral-900 border border-neutral-800 p-2 text-sm rounded-xs text-white" />
          </div>
          <div>
            <label className="block text-xs text-neutral-400 mb-1">نص الزر الأساسي</label>
            <input type="text" name="heroPrimaryLabel" defaultValue={content?.heroPrimaryLabel} className="w-full bg-neutral-900 border border-neutral-800 p-2 text-sm rounded-xs text-white" />
          </div>
          <div>
            <label className="block text-xs text-neutral-400 mb-1">شريط الإعلانات (Promo Text)</label>
            <input type="text" name="promoText" defaultValue={content?.promoText} className="w-full bg-neutral-900 border border-neutral-800 p-2 text-sm rounded-xs text-white" />
          </div>
          <button type="submit" disabled={isSaving || isUploading} className="bg-white text-black px-6 py-2 rounded-xs font-semibold text-sm hover:bg-neutral-200 transition disabled:opacity-50">
      ...
      <div className="bg-[#141414] border border-neutral-800 p-6 rounded-xs">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {featured.map((f, i) => (
            <div key={f.id} className="bg-neutral-900 p-4 border border-neutral-800 rounded-xs">
              ...
              <form onSubmit={(e) => handleFeaturedSubmit(e, f.id)} className="space-y-3">
                ...
                {f.imageUrl && (
                  <img src={f.imageUrl} className="w-full h-32 object-cover rounded-xs border border-neutral-800" alt={f.category.name} />
                )}
                <input type="file" name="imageFile" accept="image/*" className="text-xs w-full" />
                <button type="submit" disabled={isUploading} className="bg-neutral-800 hover:bg-neutral-700 text-white px-3 py-1.5 rounded-xs text-xs font-medium transition w-full">
                  رفع وتحديث الصورة
                </button>
              </form>
            </div>
